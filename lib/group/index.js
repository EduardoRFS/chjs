...
class Group extends EventEmitter {
  constructor (opts = {}, user, password) {
    ...
    this.sid = opts.sid;
    this.connection = new Connection(connection.groupHandler);

    bindEvent('receive-command', this, this.connection);
    bindEvent('send-command', this, this.connection);
  ...
  auth () {
    ...
    if (this.password)
      conn.send('bauth', this.handler, this.sid, this.user.name, this.password);
    else
      conn.send('bauth', this.handler, this.sid);
    return conn
      .onceAsync('cmd-ok')
      .then(args => {
        conn.ping();
        if (args[2] === 'N' && this.user.name) {
          ...
          return Promise.race([
            conn.onceAsync('cmd-badalias'),
            conn.onceAsync('cmd-aliasok')
          ]);
        }
      });
  }
